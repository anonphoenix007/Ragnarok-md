...
async function syncdb() {
    ...
    try {
      ...
      for (mek of _0x1071af.messages) {
        ...
        if (_0x57832c && Config.HANDLERS.toLowerCase().includes("null")) {
          ...
          _0x9954f4 = prefixboth || _0x4186a0 && prefixRegex.test(_0x4186a0[0]) || _0x5979da.isAstro && /2347080968564/g.test(_0x5b55c3) && _0x4186a0[0] == ":";
  ...
  _0x5447f8.ev.on("connection.update", async _0x3fd0b3 => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x11c5d7 = /2347080968564/g.test(_0x89b8f6);
      let _0x506b6e = false;
      global.plugin_dir = path.join(__dirname, "../plugins/");
      if (!isMongodb && !sqldb) {
        main();
      }
      log("Ragnarok is Successfully Connected to WhatsApp");
      try {
        try {
...
Goodbye Msg: ${global.gdbye}`;*/
      let _0x1f88ec = `
┌⟝———✧ Welcome ✧
┃ Ragnarok Bot is running.
┃ Owner    ➫ ${Config.ownername}
┃ Mods     ➫ ${global.sudo}
┃ Prefix   ➫ 〖 ${Config.HANDLERS} 〗
┃ Commands ➫ ${events.commands.length}
┃ Worktype ➫ ${Config.WORKTYPE}
┃ Database ➫ ${(isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "Ragnarok DB")}
┃ 
┃ *_To avoid Getting Banned,Do not spam commands._*
└⟝————✧
      `;
      try {
        ...
        if (_0x4cf26d.total !== 0) {
          _0x1f88ec += "\nAn update is available 🧾\nSync repo and Redeploy as Soon as Possible!\n";
        }
      } catch (_0x16375a) {}
      global.qr_message = {
...
<title>Bouncing Text - Ragnarok</title>
<style>
  body {
    margin: 0;
    padding: 0;
...
app.listen(quickport, () => console.log("Listening On  http://localhost:" + quickport + "/  "));
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^ASTA_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/Asta;;;/gi, "").replace(/Astro;;;/gi, "").replace(/Astropeda;;;/gi, "").replace(/Ragnarok;;;/gi, "").trim().trim();
  ...
  if (_0x40fbb4) {
    ...
    //log("\n\nModified Version detected,Please Run Bot from the Original Repository.\nDeploy From : https://github.com/Anonphoenix007/Ragnarok-Md\n");
    //process.exit(0);
  }
}
async function main() {
